...
import { CheckCircle, User, Trophy, Calendar, MapPin, Upload, FileText } from "lucide-react"
...
export function CompetitionRegistrationForm() {
  ...
  const steps = [
    ...
    { number: 4, title: "Tinjau & Kirim", icon: CheckCircle },
  ]
  ...
  const checkStep = (step: number): { valid: boolean; errors: Record<string, string> } => {
    ...
    if (step === 1) {
      ...
      if (!formData.noHandphone.trim()) e.noHandphone = "No handphone/WhatsApp wajib diisi"
    }

    if (step === 2) {
      if (!formData.tingkatPendidikan) e.tingkatPendidikan = "Tingkat pendidikan wajib dipilih"
      if (!formData.category) e.category = "Kategori kompetisi wajib dipilih"
    }
    ...
    <div className="max-w-4xl mx-auto px-4">
      <div className="text-center mb-6 sm:mb-8">
        ...
        <p className="text-white/80 text-sm sm:text-base md:text-lg px-2">
          Bergabunglah dengan Stibajra Technology Competition. Daftar sekarang untuk mengamankan tempat Anda.
        </p>
        ...
        {steps.map((step, index) => {
          ...
          return (
            <div key={step.number} className="flex flex-col items-center flex-1 relative">
              ...
              <span
                className={`text-xs sm:text-sm font-medium text-center px-1 leading-tight ${
      ...
      <Card className="glass-card border-white/30">
        <CardHeader className="pb-4 sm:pb-6">
          ...
          <CardDescription className="text-white/80 text-xs sm:text-sm md:text-base">
            ...
            {currentStep === 2 && "Pilih tingkat pendidikan dan kategori Anda"}
            {currentStep === 3 && "Upload dokumen yang diperlukan"}
            {currentStep === 4 && "Tinjau informasi Anda sebelum mengirim"}
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4 sm:space-y-6 px-4 sm:px-6">
          ...
          {currentStep === 2 && (
            <div className="space-y-3 sm:space-y-4">
              <div className="space-y-2">
                <Label htmlFor="tingkatPendidikan" className="text-white text-sm">
                  Tingkat Pendidikan *
                </Label>
                <Select
                  value={formData.tingkatPendidikan}
                  onValueChange={(value) => handleInputChange("tingkatPendidikan", value)}
                >
                  <SelectTrigger className="glass border-white/30 text-white text-sm sm:text-base">
                    <SelectValue placeholder="Pilih tingkat pendidikan Anda" />
                  </SelectTrigger>
                ...
                {errors.tingkatPendidikan && <p className="text-red-300 text-xs">{errors.tingkatPendidikan}</p>}
              </div>
              <div className="space-y-2">
              ...
              <div className="space-y-2">
                ...
                <div className="glass border-white/30 border-2 border-dashed rounded-lg p-4 text-center">
                  ...
                  <Label htmlFor="buktiFollow" className="cursor-pointer">
                    <span className="text-white/80 text-sm">
                      {formData.buktiFollow ? formData.buktiFollow.name : "Klik untuk upload bukti follow"}
                    </span>
                    <p className="text-white/60 text-xs mt-1">Format: JPG, PNG, PDF (Max 5MB)</p>
                  </Label>
                </div>
